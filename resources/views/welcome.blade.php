        ...
        @foreach($medicines as $medicine)
            ...
            <div class="col-md-3 mt-4">
                <div class="card-product">
                    <a href="/medicine/{{$medicine->id}}"><img src="{{ $medicine->image }}" width="150px" alt="products" srcset=""></a>
                    <div class="card-product-desc">
                        <a href="/medicine/{{$medicine->id}}" class="mt-3" style="text-decoration: none; color: black;"><h3>{{ $medicine->name }}</h3></a>
                        <span>@currency($medicine['price']) Per {{ $medicine->unit }}</span>
                        <br>
                        <span>Stock {{$medicine['stock']}}</span>
                        <br>
                        <hr>
                        <form action="/cart?medicine_id={{ $medicine->id }}" method="post">
                            @csrf
